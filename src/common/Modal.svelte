...
<script lang="ts">
	...
	const camelCaseToDash = (str: any) => str.replace(/([a-zA-Z])(?=[A-Z])/g, '$1-').toLowerCase();

	const toCssString = (props: any) =>
		props
	...
	const handleKeydown = (event: any) => {
		...
		if (Component && event.key === 'Tab' && !state.disableFocusTrap) {
			...
			const tabbable = Array.from(nodes).filter((node: any) => node.tabIndex >= 0);
	...
	const handleOuterMousedown = (event: any) => {
		if (state.closeOnOuterClick && (event.target === background || event.target === wrap))
			outerClickTarget = event.target;
	};

	const handleOuterMouseup = (event: any) => {
