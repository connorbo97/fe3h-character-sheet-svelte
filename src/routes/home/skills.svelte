...
<div class="container">
    ...
    {@const skillStatModifier = getModifierByPlayerStat(stats[skillStat])}
      ...
      <button class={`${skillProficiencyMod ? 'blue' : 'white'}`} on:click={() => onToggleSkillProficiency(skill)} />
      <div class="total">{skillBonusMod + skillProficiencyMod + skillStatModifier}</div>
			<div class="label">{`${PLAYER_SKILL_TO_LABEL[skill]} (${PLAYER_STAT_TO_SHORT_LABEL[skillStat]})`}</div>
		</div>
	{/each}
</div>
